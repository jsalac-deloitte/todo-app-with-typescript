...
const Todo: React.FC<task> = ({
  ...
  return (
    <div className="flex items-center justify-between px-2 space-x-2 bg-blue-700 backdrop-blur-sm bg-opacity-20 rounded-lg hover:bg-yellow-200 py-2 border-b">
      <input type="checkbox" onClick={toggleTask} />
      <div className="text-xl">{id}</div>
      <div className={`grow text-lg   ${completed ? "line-through" : ""}`}>
        {task}
      </div>
